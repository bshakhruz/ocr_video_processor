...
class BOXES_HELPER():
    ...
    def show_boxes_words(self, d, frame, font_path):
        text_vertical_margin = 12
        n_boxes = len(d['level'])
        for i in range(n_boxes):
            if (int(float(d['conf'][i])) > 60) and not (d['text'][i].isspace()):  # Words
                ...
                draw.text((x, y - text_vertical_margin), d['text'][i], font=font, fill=(0, 255, 0, 0))
...
class OCR_HANDLER:
    ...
    def __init__(self, video_filepath, cv2_helper, ocr_type="WORDS", target_language="ru"):
    ...
    def process_frames(self):
        ...
        frame_name = os.path.join(self.frames_folder, self.video_name + '_frame_')
        ...
        if not video.isOpened():
            raise ValueError(f"Error opening video file: {self.video_filepath}")
    ...
    def ocr_frame(self, frame):
        ...
        if d is None or not d.get('level'):
            print("Warning: Tesseract did not return valid data for a frame.")
            return frame


        if (self.ocr_type == "LINES"):
            lines_with_words = self.boxes_helper.get_lines_with_words(self.boxes_helper.get_organized_tesseract_dictionary(d))
            for line in lines_with_words:
                line['text'] = self.translate_text(line['text'])
            frame = self.boxes_helper.show_boxes_lines(d, frame)
        else:
            n_boxes = len(d['level'])
            for i in range(n_boxes):
                if int(float(d['conf'][i])) > 60 and not d['text'][i].isspace():
                    d['text'][i] = self.translate_text(d['text'][i])
            frame = self.boxes_helper.show_boxes_words(d, frame, self.font_path)
    ...
    def compute_best_preprocess(self, frame):
        ...
        for idx, opt in enumerate(options):
            ...
            if d is None or not d.get('text'):
                continue
            ...
            current_f = f(len(confs), mean_conf)
            if current_f > best_f:
                best_f = current_f
                best_im = im
                best_d = d
